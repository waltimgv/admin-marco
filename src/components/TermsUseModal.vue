<template>
  <v-dialog v-model="modal" :persistent="is_terms_use_accepted" width="600px">
    <v-card>
      <v-card-title>
        <span class="headline">Terms of Use</span>
      </v-card-title>
      <v-card-text>
      ...
      <v-card-actions >
        <v-spacer></v-spacer>
        <template v-if="is_terms_use_accepted">
          <v-btn color="red darken-1" @click="disagree">Disagree</v-btn>
          <v-btn color="green darken-1" @click="agree">I agree with terms of use.</v-btn>
        </template>
        <template v-else>
          <v-btn color="green darken-1" @click="modal = false">Close</v-btn>
        </template>
      </v-card-actions>
    </v-card>
  </v-dialog>
...
<script>
  ...
  export default {
    name: 'TermsUseModal',
    computed: {
      ...mapState(['termsuse_modal', 'user']),
      modal: {
        get() {
          return this.termsuse_modal;
        },
        set(val) {
          this.setTermsUseModal(val);
        },
      },
      is_terms_use_accepted() {
        return this.user && this.user.is_terms_use_accepted === false
      }
    },
    methods: {
      ...mapMutations({
        ...
        setUserTermsUse: 'SET_USER_TERMS_USE',
      }),
      disagree() {
        const vm = this;
